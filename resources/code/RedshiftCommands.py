...
try:
    ...
    # Add retry to handle unexpected cluster connection timeout.
    retry = 3
    while retry > 0:
        try:
            con1 = pgdb.connect(database=database, host=host, user=user, password=password, port=port)
            con1.close()
            print('Redshift cluster is up and running: %s' % host)
            break
        except Exception as error:
            print(error)
            retry = retry - 1
            if retry <= 0:
                print('Cannot connect to Redshift cluster: %s' % host)
                raise
